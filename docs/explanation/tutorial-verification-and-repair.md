...
`tutorial_extract_blocks` reads that array with jq, keeps the steps whose `language` appears in `verify.executable_languages`, and writes one `step-N.sh` per surviving block, where N is the step's index in the array. Three details in that function are load-bearing. It gets one jq call to list the executable block indices and then one jq call per index that redirects that block's code straight into its script, so the code never transits a shell variable and a tutorial containing tabs, newlines or quotes cannot alter how it is extracted. It deletes any `step-*.sh` from a previous attempt first, so a repair that removes a step does not leave the old script behind to be executed anyway. And for `console` blocks jq keeps only lines beginning with `$ `, on the convention that a console transcript interleaves commands with their output, so everything else in that block is output to display rather than a command to run.

An earlier version of this function base64-encoded each field through a single tab-delimited pipeline and decoded on the other side, which achieved the same quoting safety. It was replaced because decoding encoded content into a `.sh` file that is then executed is indistinguishable, to an endpoint protection product, from the malware pattern of the same shape, and it generated antivirus alerts on developer machines. The per-index form has no encoding step to flag.
...
Finally, the harness default is host execution. Absent `verify.sandbox_command`, model-generated shell runs on the machine invoking `diataxis generate`, in an empty directory but with your user's full authority. The empty directory limits accidents, not intent, and `verify.mode: "sandbox"` on its own does not change this: the mode selects verification-in-a-wiped-directory, and the confinement comes from `sandbox_command`. A config that sets the mode without the command is not sandboxed, which is a trap worth stating plainly because the key name suggests otherwise.

This repository's own config therefore sets `sandbox_command` to the wrapper shipped at `share/sandbox/verify-sandbox.sh`. It runs each step under `sandbox-exec` on macOS or `bwrap` on Linux, with no network, no writes outside the verification directory, and the harness's own run directory explicitly denied so one tutorial's step cannot rewrite the next page's script. When neither mechanism exists it exits 78 rather than running the step unconfined; a sandbox that quietly stops sandboxing is worse than one that refuses.
...
| Where tutorial code executes | A wiped empty directory, with verify.sandbox_command as the opt-in confinement hook | The repository working tree | A newcomer starts from nothing, so a tutorial that assumes a prepared workspace should fail; the wrapper covers the cases where a toolchain genuinely has to pre-exist |
| What happens when no sandbox mechanism is available | share/sandbox/verify-sandbox.sh exits 78 and the step does not run | Fall back to unconfined host execution so verification still completes | A security control that silently degrades is worse than one that fails loudly; the operator can choose host execution by clearing sandbox_command, but not by accident |
| Whether the per-user temp dir stays writable under the sandbox | Yes, because macOS mktemp bypasses TMPDIR via confstr and tutorial shell uses it constantly | Deny it for a tighter boundary | Denying it makes tutorials fail for reasons unrelated to their content, and the realistic outcome is operators disabling the sandbox entirely; the harness's own run directory is denied separately, which is the part that actually mattered |
