...
The most interesting soft edge is that the boundary relocates one risk rather than eliminating it. Tutorials are verified by executing their fenced code blocks, and the harness does that execution itself, in an ephemeral directory, by default on the host. So model-authored shell does run on your machine during a tutorial generation. The difference from handing the model a Bash tool is control: the harness decides when, in which directory, in which order, and whether to wrap each script in `verify.sandbox_command`. That knob being opt-in rather than mandatory is a real tradeoff, made because requiring confinement would make the default path fail on machines without a mechanism for it.

Opt-in does not mean you should leave it off. `share/sandbox/verify-sandbox.sh` ships with the harness and needs no container runtime, using `sandbox-exec` on macOS and `bwrap` on Linux, so pointing `sandbox_command` at it is close to free; this repository's own config does exactly that. Note also that `verify.mode: "sandbox"` alone confines nothing, which is easy to misread from the key name: the mode picks verification in a wiped directory, and only `sandbox_command` supplies a boundary.
...
- Tutorial verification executes model-authored shell on the host by default; verify.sandbox_command is opt-in. Now that share/sandbox/verify-sandbox.sh ships in-tree and depends only on sandbox-exec or bwrap rather than a container runtime, the argument for leaving the default unconfined is weaker than it was. Whether it should become the default, at the cost of failing on hosts with neither mechanism, is an open call.
